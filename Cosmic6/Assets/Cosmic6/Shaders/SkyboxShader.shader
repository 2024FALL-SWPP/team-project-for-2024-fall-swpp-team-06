...
Properties {
	...
	_Exposure("Exposure", Range(0, 30)) = 1.3
	_SunExposure("SunExposure", Range(0, 200)) = 1

	_TextureOrGradient ("Texture or Gradient", Range (0, 1)) = 0
	_GradientVerticalOffset ("Gradient Vertical Offset", Range (-1, 1)) = 0
	_GradientExponent ("Gradient Exponent", Range (0, 40)) = 1
	_GradientColorStart ("Gradient Color Start", Color) = (.22, .19, .31, 1)
	_GradientColorEnd ("Gradient Color End", Color) = (.0, .0, .0, .0)
	[Gamma] _GradientExposure ("Gradient Exposure", Range(0, 30)) = 1



	_Tint("Tint Color", Color) = (.5, .5, .5, .5)
	[Gamma] _ExposureCube("Exposure", Range(0, 30)) = 1
	_Rotation("Rotation", Range(0, 360)) = 0
	_RotationPitch("RotationPitch", Range(-90, 90)) = 0
	[NoScaleOffset] _Tex("Cubemap   (HDR)", Cube) = "grey" {}

	_Ambient("Ambient", Color) = (0,0,0,0)

	_Tint2("Tint Color 2", Color) = (.5, .5, .5, .5)
	[Gamma] _ExposureCube2("Exposure 2", Range(0, 30)) = 1
	_Rotation2("Rotation 2", Range(0, 360)) = 0
	_RotationPitch2("RotationPitch 2", Range(-180, 180)) = 0
	[NoScaleOffset] _Tex2("Cubemap   (HDR) 2", Cube) = "grey" {}
	
	_Tint3("Tint Color 3", Color) = (.5, .5, .5, .5)
	[Gamma] _ExposureCube3("Exposure 3", Range(0, 30)) = 1
	_Rotation3("Rotation 3", Range(0, 360)) = 0
	_RotationPitch3("RotationPitch 3", Range(-180, 180)) = 0
	[NoScaleOffset] _Tex3("Cubemap (HDR) 3", Cube) = "grey" {}
	
	_Tint4("Tint Color 4", Color) = (.5, .5, .5, .5)
	[Gamma] _ExposureCube4("Exposure 4", Range(0, 30)) = 1
	_Rotation4("Rotation 4", Range(0, 360)) = 0
	_RotationPitch4("RotationPitch 4", Range(-180, 180)) = 0
	[NoScaleOffset] _Tex4("Cubemap (HDR) 4", Cube) = "grey" {}
	
	[NoScaleOffset] _TexMask("Cubemap   (HDR) Mask", Cube) = "white" {}
...
SubShader{
	...
	Pass {
		...
		uniform half _SunExposure;
		uniform half3 _GroundColor;
		uniform half _SunSize;
		uniform half3 _SkyTint;
		uniform half _AtmosphereThickness;

		float _TextureOrGradient;
		float _GradientVerticalOffset;
		float _GradientExponent;
		half _GradientExposure;
		...
		half _ExposureCube4;
		float _Rotation4;
		float _RotationPitch4;

		samplerCUBE _TexMask;
		half4 _TexMask_HDR;
		...
		static const float kKmESun = kMIE * kSUN_BRIGHTNESS  * _SunExposure;
		...
		struct v2f
		{
			...
			float3 texcoord2 : TEXCOORD4;
			float3 texcoord3 : TEXCOORD7;
			float3 eyeRay : TEXCOORD6;
		};
		...
		float3x3 CreateRotationAroundX(float degrees)
		{
			float alpha = degrees * UNITY_PI / 180.0;
			float sina, cosa;
			sincos(alpha, sina, cosa);
		...
		float3x3 CreateRotationAroundY(float degrees)
		{
			float alpha = degrees * UNITY_PI / 180.0;
			float sina, cosa;
			sincos(alpha, sina, cosa);
		...
		float4 RotateAroundXYInDegrees(float4 vertex, float degreesYaw, float degreesPitch)
		{
			float3x3 rotPitch = CreateRotationAroundX(degreesPitch);
			float3x3 rotYaw = CreateRotationAroundY(degreesYaw);

			float3 rotatedVertex = mul(rotPitch, vertex.xyz);
			rotatedVertex = mul(rotYaw, rotatedVertex);
		...
		{
			v2f OUT;
			OUT.eyeRay = v.vertex.xyz;
			OUT.pos = UnityObjectToClipPos(v.vertex);


			//OUT.vertex = mul(UNITY_MATRIX_MVP, RotateAroundYInDegrees(v.vertex, _Rotation));
			OUT.texcoord3 = RotateAroundXYInDegrees(v.vertex, _Rotation3, _RotationPitch3).xyz;
			OUT.texcoord2 = RotateAroundXYInDegrees(v.vertex, _Rotation2, _RotationPitch2).xyz;
			OUT.texcoord = RotateAroundXYInDegrees(v.vertex, _Rotation,_RotationPitch).xyz;
			//v.vertex = mul(UNITY_MATRIX_MVP, RotateAroundYInDegrees(v.vertex, _Rotation));


			//OUT.texcoord = v.vertex.xyz;
			//OUT.texcoord2 = v.vertex.xyz;
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		half4 frag (v2f IN) : SV_Target
		{
			...
			half4 texMask = texCUBE(_TexMask, IN.texcoord3);
			half3 planetMask = texMask.rgb;
		...
		// if y < 0 [eyeRay.y > 0] - sky
			...
			if(y < 0.0)
			{
				...
				col += (mie * IN.sunColor) * (1-planetMask);
			}
		#endif
			...
			half3 c1 = DecodeHDR(tex, _Tex_HDR);
			c1 = c1 * _Tint.rgb * unity_ColorSpaceDouble.rgb + _Ambient;
			c1 *= _ExposureCube;

			// 1 at _GradientVerticalOffset, 0 at distance 1 and higher from _GradientVerticalOffset.
			//float t = lerp (1, 0, saturate(abs(IN.eyeRay.y - _GradientVerticalOffset)));
			float t = 1 - saturate(abs(IN.eyeRay.y - _GradientVerticalOffset));
			t = pow (t, _GradientExponent);
			half3 c1prim = _GradientExposure * lerp (_GradientColorEnd.rgb, _GradientColorStart.rgb, t);
			
			c = lerp (c1, c1prim, _TextureOrGradient);

			half4 tex2 = texCUBE(_Tex2, IN.texcoord2);
			half3 c2 = DecodeHDR(tex2, _Tex2_HDR);
			c2 = c2 * _Tint2.rgb * unity_ColorSpaceDouble.rgb;
			c2 *= _ExposureCube2;

			half4 tex3 = texCUBE(_Tex3, IN.texcoord3);
			...
			return half4((col+c+c2*(1-planetMask)+c3*planetMask)*(1-c)*0.23,1.0);
